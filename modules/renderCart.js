...
const renderCartList = async () => {
  ...
  if (cartControl.cartData.length) {
      ...
      cartControl.cartData.map(
          async (item) => 
              await getData(
                  `https://beautiful-wry-addition.glitch.me/api/products/${item.id}`,
              ),
          ),
      );
...
const submitOrder = async (e) => {
  ...
  try {
    const response = await fetch('https://beautiful-wry-addition.glitch.me/api/orders/', {
      method: 'POST',
      headers: {
    ...
    cartForm.innerHTML = `
    <h3>Ваш заказ оформлен! Номер заказа ${order.orderId}</h3>
    `;
...
export const modalCartController = () => {
    ...
    cartListElem.addEventListener('click', dellCartItem);
